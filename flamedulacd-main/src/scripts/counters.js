...
export function renderMetrics(items = metricsData) {
  ...
  items.forEach((metric) => {
    ...
    number.dataset.target = String(Number(metric.value) || 0);
    number.dataset.mode = metric.mode || 'number';
    number.textContent = '0';

    const label = document.createElement('h4');
    label.style.cssText = 'font-size:1rem;font-weight:600;color:white;margin-bottom:4px;';
    label.textContent = metric.label || 'Indicador FlaMedula';

    const description = document.createElement('span');
    description.style.cssText = 'font-size:0.8rem;color:rgba(255,255,255,0.6);line-height:1.2;';
    description.textContent = metric.description || metric.suffix || '';

    card.append(number, label, description);
...
function animateMetrics(container) {
  container.querySelectorAll('.metric-number').forEach((element) => {
    ...
    function update(currentTime) {
      ...
      else element.innerText = `${target}${element.dataset.mode === 'percentage' ? '%' : ''}`;
    }
