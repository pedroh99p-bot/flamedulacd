...
export function renderTeam(items = teamMembers) {
...
export function renderAmbassadors(items = ambassadors) {
  const grid = document.getElementById('ambassadorsGrid');
  if (!grid) return;

  grid.classList.toggle('has-single-item', items.length === 1);
  grid.innerHTML = items.map((ambassador) => {
    const avatar = createAvatarMarkup(ambassador, 'ambassador-avatar');

    return `<div class="ambassador-card"><div class="ambassador-badge">${escapeHtml(ambassador.role)}</div>${avatar}<h3 style="font-size:1.1rem; font-weight:700;">${escapeHtml(ambassador.name)}</h3><p class="ambassador-desc">"${escapeHtml(ambassador.description || ambassador.desc || '')}"</p></div>`;
  }).join('');
}
...
export function renderTestimonials(items = testimonialsData) {
  const track = document.getElementById('testimonialsTrack');
  if (!track) return;

  track.innerHTML = items.map((testimonial) => {
    const secondary = [testimonial.role, testimonial.case].filter(Boolean).map(escapeHtml).join(' \u2022 ');
    return `
      <div class="testimonial-card">
        <div class="testimonial-header">
          <div class="testimonial-avatar">${escapeHtml(testimonial.avatar)}</div>
          <div class="quote-mark">"</div>
        </div>
        <p class="testimonial-text">"${escapeHtml(testimonial.text)}"</p>
        <div class="testimonial-author-info">
          <p class="testimonial-author">${escapeHtml(testimonial.name)}</p>
          ${secondary ? `<p class="testimonial-role">${secondary}</p>` : ''}
        </div>
      </div>
    `;
  }).join('');
