...
export function getPublishedHero() {
  return remember('hero_news:published', () => fetchRest('v_public_hero_news', {
...
export function getPublishedActions() {
  return remember('actions:published', () => fetchRest('v_public_actions', {
...
export function getPublishedMediaItems() {
  return remember('media_items:published', () => fetchRest('v_public_media_items', {
...
export function getPublishedTestimonials() {
  return remember('testimonials:published', () => fetchRest('v_public_testimonials', {
    select: 'id,quote,author_name,author_label,image_asset_id,image_url,image_alt,cloudinary_public_id,published,sort_order,updated_at',
    published: 'eq.true',
    order: 'sort_order.asc.nullslast,updated_at.desc.nullslast',
  }, { code: 'TESTIMONIALS_REQUEST_FAILED' }));
}

export function getPublishedTeamMembers() {
  return remember('team_members:published', () => fetchRest('v_public_team_members', {
    select: 'id,name,role,description,member_type,image_asset_id,image_url,image_alt,cloudinary_public_id,published,sort_order,updated_at',
    published: 'eq.true',
    order: 'sort_order.asc.nullslast,updated_at.desc.nullslast',
...
export function getPublishedFaqItems() {
  return remember('faq_items:published', () => fetchRest('v_public_faq_items', {
    select: 'id,question,answer,category,published,sort_order,updated_at',
    published: 'eq.true',
    order: 'sort_order.asc.nullslast,updated_at.desc.nullslast',
  }, { code: 'FAQ_REQUEST_FAILED' }));
}

export function getPublishedTransparencyMetrics() {
  return remember('transparency_metrics:published', () => fetchRest('v_public_transparency_metrics', {
    select: 'id,key,label,value,description,mode,published,sort_order,updated_at',
    published: 'eq.true',
    order: 'sort_order.asc.nullslast,updated_at.desc.nullslast',
...
export function getMediaAssetsByIds(ids = []) {
  ...
  return remember(cacheKey, async () => {
    try {
      const rows = await fetchRest('v_public_media_assets', {
