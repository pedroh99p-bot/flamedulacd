...
- Limite excedido: `429` com `code: "RATE_LIMIT_EXCEEDED"`, `Retry-After` e cabecalhos `X-RateLimit-*`.
- Limitador indisponivel ou sem configuracao segura: `503` com `code: "SERVICE_UNAVAILABLE"`.
...
- Cada endpoint aceita inicialmente 5 tentativas por identificador em 10 minutos. O contador e compartilhado no Postgres e a chave usa SHA-256 com salt; o IP bruto nao e persistido.
...
O ambiente das Edge Functions deve definir `RATE_LIMIT_SALT` com pelo menos 32 caracteres aleatorios. `RATE_LIMIT_FAIL_OPEN` deve permanecer ausente/`false` em producao; quando `true`, uma falha do limitador permite o envio e deve ser tratada apenas como contingencia monitorada.
...
- `amount` obrigatorio, numero maior que zero; representa somente o valor pretendido.
...
- `submission_mode`: `pre_pix`
- `document_type`, `document` e `email`: `null`; o sistema nao inventa dados pessoais.
- `contact_preference`: `whatsapp`
- `payment_method`: `pix`
- `donation_type`: `single`
- `amount`: `null`; nao existe pagamento confirmado nesta etapa.
- `intended_amount`: valor informado pelo visitante.
- `source`: `apoie_page`
...
- `source_section`: secao de origem do formulario.
- `internal_notes`: `null`.
