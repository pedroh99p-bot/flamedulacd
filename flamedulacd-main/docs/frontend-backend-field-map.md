...
| Campo visual | `name`/origem no frontend | Campo no payload | Coluna Supabase |
| --- | --- | --- | --- |
| Modo de submissao | valor fixo no builder | `submission_mode = pre_pix` | `submission_mode` |
...
| Tipo de documento | backend | nao enviado no modo `pre_pix` | `document_type = null` |
| CPF/CNPJ | backend | nao enviado no modo `pre_pix` | `document = null` |
| E-mail | backend | nao enviado no modo `pre_pix` | `email = null` |
...
| Valor pretendido | `amount_display` | `amount` | `intended_amount` |
| Valor confirmado | backend/provedor futuro | nao enviado no modo `pre_pix` | `amount = null` |
